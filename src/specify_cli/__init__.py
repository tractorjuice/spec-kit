    ...
    with Live(tracker.render(), console=console, refresh_per_second=8, transient=True) as live:
        ...
        try:
            ...
            if selected_ai == "codex":
                tracker.add("envrc", "Create .envrc for Codex")
                tracker.start("envrc")
                try:
                    envrc_path = project_path / ".envrc"
                    envrc_content = f"""# Auto-generated by specify CLI for Codex CLI support
...
"""
                    ...
                    # Also create/update .gitignore for Codex
                    ...
                    tracker.complete("envrc", ".envrc created")
                except Exception as e:
                    tracker.error("envrc", str(e))
    ...
    if selected_ai == "codex":
        steps_lines.append(f"{step_num}. Set up [cyan]CODEX_HOME[/cyan] environment variable (required for Codex):")
        steps_lines.append(f"   {step_num}.1 [cyan]RECOMMENDED[/cyan]: Install direnv and run [cyan]direnv allow[/cyan] (.envrc created)")
        ...
        steps_lines.append(f"   {step_num}.2 Alternative: Run [cyan]{cmd}[/cyan] in each shell session")
        steps_lines.append(f"   {step_num}.3 See [cyan]AGENTS.md#codex-cli-setup[/cyan] for more options")
        step_num += 1
